...
def train(epoch):
    ...
    for batch_idx, X in enumerate(train_loader):
        ...
        seen += len(target)
        print(
            "Train Epoch {}: batch {}/{} [{}/{} ({:.2f}%)]   Loss: {:.6f}\r".format(
                epoch,
                batch_idx + 1,
                len(train_loader),
                seen,
...
def print_label_indices(codes):
    label_indices = codes.label_index
    print("Label indices:")
    for label, index in label_indices.items():
        print(f"{label}: {index}")


def get_label_indices(codes, labels):
    label_indices = codes.label_index
    indices = {
        label: label_indices[label] for label in labels if label in label_indices
    }
    return indices


print_label_indices(codes)

labels_of_interest = ["B-drug_n", "I-drug_n"]
drug_n_indices = get_label_indices(codes, labels_of_interest)

n_labels = codes.get_n_labels()
class_weights = torch.ones(n_labels)

for label, index in drug_n_indices.items():
    class_weights[index] = 35.0
